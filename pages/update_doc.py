...
def app():
    with st.form("my_form"):
        ...
        types = st.text_input("id number")

        submitted = st.form_submit_button("Submit")

        if submitted:
            response = docs.update(title, description, why, repo_conn, tests, devs, types, puml_txt.split("\n"), id)
            st.success(f"Update To Documents: {response}")